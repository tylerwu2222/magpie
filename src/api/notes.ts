...
export const addNote = async (newNoteContent: newEntryDataType) => {
  const userID = await getSessionUserID();
...
export const deleteNoteByID = async (noteID: number | undefined) => {
  try {
    const { data, error } = await supabase
      .from("notes")
      .delete()
...
export const duplicateNoteByID = async (noteID: number | undefined) => {
